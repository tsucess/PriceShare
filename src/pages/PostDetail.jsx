  ...
  if (!post) {
    return (
      <div style={{
        ...
        <p style={{ color: theme.textMuted, fontWeight: 600, marginBottom: '20px' }}>Post not found</p>
        <button
          onClick={() => navigate('/feed')}
          style={{
            padding: '12px 24px', borderRadius: '10px', fontWeight: 700,
            background: `linear-gradient(135deg, ${theme.accent}, #00c853)`,
            color: '#0a0a0f', border: 'none', cursor: 'pointer', fontSize: '14px'
          }}
        >← Back to Feed</button>
      </div>
    );
    ...
    <div style={{
      ...
      <div style={{ padding: '20px 20px 0' }}>
        ...
        <div style={{ marginBottom: '16px' }}>
          <h1 style={{ fontSize: '22px', fontWeight: 800, color: theme.text, margin: '0 0 10px' }}>
            {post.product}
          </h1>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
            ...
            <span style={{
              fontSize: '11px', background: `${catColor}15`,
              color: catColor, padding: '4px 12px', borderRadius: '6px', fontWeight: 700
        ...
        <div style={{
          ...
          <p style={{ fontSize: '12px', fontWeight: 700, color: catColor, margin: '0 0 8px' }}>💡 Price Context</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '24px' }}>
          ...
          >{liked ? '❤️' : '🤍'} {liked ? post.likes + 1 : post.likes} Likes</button>
          ...
          <button
            ...
            style={{
              ...
              cursor: reported ? 'not-allowed' : 'pointer', transition: 'all 0.15s'
            }}
          >{reported ? '✅ Reported' : '🏛️ Report Price'}</button>
        </div>

        {/* COMMENTS SECTION */}
        <div style={{ background: theme.card, border: `1px solid ${theme.cardBorder}`, borderRadius: '16px', overflow: 'hidden', marginBottom: '24px' }}>
          ...
          <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '300px', overflowY: 'auto' }}>
            {comments.length === 0 ? (
              ...
              comments.map((c, i) => (
                <div key={i} style={{
                  background: theme.pill, borderRadius: '12px', padding: '12px 14px',
                  display: 'flex', flexDirection: 'column', gap: '4px'
                }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '12px', fontWeight: 700, color: catColor }}>
                      👤 {c.user || 'Anonymous'}
                    </span>
                    <span style={{ fontSize: '11px', color: theme.textDim }}>{c.date || post.date}</span>
                  </div>
                  <p style={{ fontSize: '13px', color: theme.text, margin: 0 }}>{c.text || c}</p>
                </div>
              ))
            )}
          </div>
          ...
          <div style={{ padding: '12px 16px', borderTop: `1px solid ${theme.cardBorder}`, display: 'flex', gap: '8px' }}>
            ...
            <button
              ...
              style={{
                padding: '11px 18px', background: `linear-gradient(135deg, ${theme.accent}, #00c853)`,
                color: '#0a0a0f', borderRadius: '10px', border: 'none',
