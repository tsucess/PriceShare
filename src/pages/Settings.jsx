...
function Settings() {
  ...
  const Section = ({ title, children }) => (
    <div style={{
      ...
      borderRadius: '16px', overflow: 'hidden', marginBottom: '16px', transition: 'all 0.3s',
    }}>
      <div style={{
  ...
  const Row = ({ icon, label, desc, right }) => (
    <div style={{
      ...
      padding: '14px 18px', borderBottom: `1px solid ${theme.cardBorder}`, gap: '12px',
      flexWrap: 'wrap',
    }}>
        ...
        <div style={{ maxWidth: '600px' }}>
          ...
          <Section title="Privacy & Security">
            <Row icon="📍" label="Show My Location" desc="Others can see which city your reports are from"
              right={<Toggle value={privacy.showLocation} onChange={(v) => { setPrivacy({ ...privacy, showLocation: v }); showToast(v ? 'Location visible to others 📍' : 'Location hidden', v ? 'success' : 'info'); }} />}
            />
          ...
          <Section title="Two-Factor Authentication">
            <Row
              ...
              right={<Toggle value={twoFA} onChange={(v) => { setTwoFA(v); showToast(v ? '2FA enabled! Your account is more secure 🛡️' : '2FA disabled', v ? 'success' : 'warning'); }} />}
            />
            {twoFA && (
              <div style={{ padding: '16px 18px', background: `${theme.accent}08`, borderBottom: `1px solid ${theme.cardBorder}` }}>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  <HapticButton
                    ...
                    style={{ padding: '9px 18px', borderRadius: '9px', fontSize: '13px', fontWeight: 700, background: `linear-gradient(135deg, ${theme.accent}, #00c853)`, color: '#0a0a0f', border: 'none' }}
                  >📱 Set Up via SMS</HapticButton>
                  <HapticButton
                    onClick={() => showToast('Auth app setup coming soon! 🚧', 'info')}
                    style={{ padding: '9px 18px', borderRadius: '9px', fontSize: '13px', fontWeight: 700, border: `1px solid ${theme.cardBorder}`, background: 'transparent', color: theme.textMuted }}
                  >Use Auth App</HapticButton>
                </div>
              </div>
            )}
          </Section>
          ...
          <HapticButton
            ...
            style={{
              ...
              background: 'rgba(255,77,109,0.08)', color: '#ff4d6d', marginBottom: '12px',
            }}
          ...
          {!showDeleteConfirm ? (
            <HapticButton
              ...
              style={{
                width: '100%', padding: '16px', borderRadius: '14px', fontSize: '15px',
                fontWeight: 800, border: `1px solid ${theme.cardBorder}`,
                background: 'transparent', color: theme.textMuted, marginBottom: '40px',
              }}
            ...
            <div style={{ background: 'rgba(255,77,109,0.06)', border: '1px solid rgba(255,77,109,0.3)', borderRadius: '16px', padding: '20px', marginBottom: '40px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <HapticButton
                  ...
                  style={{ flex: 1, padding: '12px', borderRadius: '10px', fontSize: '13px', fontWeight: 700, border: `1px solid ${theme.cardBorder}`, background: 'transparent', color: theme.textMuted }}
                >Cancel</HapticButton>
                <HapticButton
